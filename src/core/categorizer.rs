...
impl CategorySummary {
    ...
    pub fn get_top_categories(&self, limit: usize) -> Vec<(PatternCategory, usize)> {
        ...
        categories.into_iter().take(limit).map(|(k, v)| (k.clone(), *v)).collect()
    }
}
...
mod tests {
    ...
    fn test_categorize_macos_pattern() {
        ...
        assert_eq!(category, PatternCategory::OperatingSystem("macOS".to_string()));
    }
    ...
    fn test_categorize_custom_pattern() {
        ...
        assert_eq!(category, PatternCategory::Custom("Project-specific".to_string()));
    }
    ...
    fn test_category_summary() {
        ...
        assert_eq!(summary.category_counts.get(&PatternCategory::Language("Python".to_string())), Some(&2));
        assert_eq!(summary.category_counts.get(&PatternCategory::Language("Node.js".to_string())), Some(&1));
        assert_eq!(summary.category_counts.get(&PatternCategory::Tool("VSCode".to_string())), Some(&1));
    }
