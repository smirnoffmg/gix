...
impl PatternAnalysis {
    ...
    pub fn new(original: String, normalized: String) -> Self {
        ...
        let matches_directories =
            matches!(pattern_type, PatternType::Directory | PatternType::Both);
    ...
    fn are_base_patterns_equivalent(&self, pattern1: &str, pattern2: &str) -> bool {
        ...
        // Handle wildcard patterns - check if one pattern matches the other
        if self.pattern_matches_wildcard(pattern1, pattern2) {
            return true;
        }
        if self.pattern_matches_wildcard(pattern2, pattern1) {
            return true;
        }
    ...
    /// Check if a pattern matches another pattern using wildcard rules
    fn pattern_matches_wildcard(&self, pattern: &str, target: &str) -> bool {
...
impl PatternAnalyzer {
    ...
    pub fn group_by_base_pattern(
        ...
        for pattern in patterns {
            ...
            // Use the normalized base pattern for grouping, but respect the analyzer's normalization setting
            let group_key = if self.normalize_patterns {
                self.normalize_pattern(&base)
            } else {
                base
            };
            groups
    ...
    pub fn get_representative_patterns(&self, patterns: &[String]) -> Vec<String> {
        ...
        for (_base, group) in groups {
            ...
            let representative = group.iter().min_by_key(|p| p.len()).unwrap().clone();
...
mod tests {
    ...
    fn test_group_by_base_pattern() {
        let analyzer = PatternAnalyzer::new(false, true); // Non-normalizing analyzer
        let patterns = vec![
            "*.log".to_string(),
            "*.log ".to_string(),
            "build/".to_string(),
        ...
        assert_eq!(groups.len(), 4); // *.log, *.log , build, and build/ groups

        let log_group = groups.get("*.log").unwrap();
        assert_eq!(log_group.len(), 1);

        let log_space_group = groups.get("*.log ").unwrap();
        assert_eq!(log_space_group.len(), 1);

        let build_group = groups.get("build").unwrap();
        assert_eq!(build_group.len(), 1);

        let build_slash_group = groups.get("build/").unwrap();
        assert_eq!(build_slash_group.len(), 1);
    }
