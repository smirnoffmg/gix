...
mod test_helpers {
    ...
    pub fn assert_entry_counts(optimized: &gix::models::gitignore::GitignoreFile, expected_entries: usize, expected_patterns: usize) {
        assert_eq!(optimized.entries.len(), expected_entries, "Expected {} entries, got {}", expected_entries, optimized.entries.len());
        assert_eq!(optimized.stats.pattern_lines, expected_patterns, "Expected {} patterns, got {}", expected_patterns, optimized.stats.pattern_lines);
    }

    /// Helper function to assert specific entry exists
    /// Makes tests more expressive about what they're checking
    pub fn assert_entry_exists(optimized: &gix::models::gitignore::GitignoreFile, expected_pattern: &str) {
        let found = optimized.entries.iter().any(|entry| entry.original == expected_pattern);
        assert!(found, "Expected pattern '{}' not found in optimized entries", expected_pattern);
    }
}
...
"#;
        ...
        assert!(optimized.entries.len() < file.entries.len(), "Optimization should reduce entry count");
        assert_eq!(optimized.stats.pattern_lines, 14, "Should have 14 unique patterns"); // Reduced from 17
        assert_eq!(optimized.stats.comment_lines, 10, "Should preserve 10 comment lines");
        assert_eq!(optimized.stats.blank_lines, 7, "Should preserve 7 blank lines");
    }
} 